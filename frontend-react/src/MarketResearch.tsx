    ...
    return (
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', background: C.bg, fontFamily: FONT }}>
        <div style={{
          ...
          <div style={{ fontSize: 13, color: '#2a2a2a', letterSpacing: '0.18em', fontFamily: FONT, textAlign: 'center' }}>
            COMPLETE A BID EVALUATION RUN TO SEE MARKET RESEARCH
          </div>
          <div style={{ fontSize: 11, color: '#1e1e1e', fontFamily: FONT, textAlign: 'center', lineHeight: 1.7 }}>
    ...
    <div style={{ flex: 1, overflowY: 'auto', background: C.bg, fontFamily: FONT }}>
      <div style={{ maxWidth: 900, margin: '0 auto', padding: '28px 28px 60px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 24 }}>
          ...
          {retrievedAt && (
            <span style={{ marginLeft: 'auto', fontSize: 10, color: '#2a2a2a', fontFamily: FONT }}>
        ...
        {loading && (
          <div style={{ fontSize: 13, color: '#2a2a2a', letterSpacing: '0.18em' }}>LOADING···</div>
        )}

        {error && (
          <div style={{
            padding: '12px 16px', background: '#1a0808', border: `1px solid #3a1010`,
            borderRadius: 2, fontSize: 13, color: C.red, marginBottom: 16,
          }}>
            ERROR: {error}
          </div>
        )}
        ...
        {status && !data && !loading && (
          <div style={{
            ...
            borderRadius: 2, fontSize: 13, color: '#555', lineHeight: 1.7,
          }}>
            {status}
          ...
          <>
            ...
            {priceRange && (
              <InfoCard title="MARKET PRICE RANGE" badge={<ConfidenceChip confidence={priceRange.confidence} />}>
                <div style={{ fontSize: 14, color: '#999', lineHeight: 1.7, fontFamily: FONT, marginBottom: priceRange.sources?.length ? 10 : 0 }}>
                  {priceRange.summary}
                </div>
                {priceRange.sources && priceRange.sources.length > 0 && (
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 4, marginTop: 8 }}>
                    {priceRange.sources.map((s, i) => (
                      <div key={i} style={{ fontSize: 11, color: '#3a3a3a', fontFamily: FONT }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
                  {Object.entries(typicalTerms).map(([key, term]) => (
                    <div key={key} style={{
                      ...
                      <div style={{ fontSize: 13, color: '#888', lineHeight: 1.6, fontFamily: FONT }}>
                        {term.summary}
                      </div>
                    </div>
                  ))}
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                  {vendorLandscape.map((sig, i) => (
                    <div key={i} style={{
                      ...
                      {sig.vendor && (
                        <div style={{ fontSize: 11, color: C.text, fontFamily: FONT, fontWeight: 600, marginBottom: 3 }}>
                          {sig.vendor}
                        </div>
                      )}
                      <div style={{ fontSize: 13, color: '#888', lineHeight: 1.6, fontFamily: FONT }}>
                      ...
                      {sig.source && (
                        <div style={{ fontSize: 10, color: '#3a3a3a', fontFamily: FONT, marginTop: 4 }}>
                          Source: {sig.source}
                        </div>
                      )}
                    </div>
            ...
            {keyRisks?.summary && (
              <InfoCard title="LIFECYCLE & CONSUMABLE COSTS">
                <div style={{ fontSize: 14, color: '#999', lineHeight: 1.7, fontFamily: FONT, marginBottom: 8 }}>
                  {keyRisks.summary}
                </div>
                ...
                {trends.signals && trends.signals.length > 0 && (
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
                    {trends.signals.map((s, i) => (
                      <div key={i} style={{ fontSize: 13, color: '#666', fontFamily: FONT, lineHeight: 1.5 }}>
                        · {s}
                      </div>
                    ))}
                  </div>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                  {redFlags.map((f, i) => (
                    <div key={i} style={{
                      ...
                      borderRadius: 2, fontSize: 13, color: '#cc7777', lineHeight: 1.6, fontFamily: FONT,
                    }}>
                      ⚑ {f}
                    </div>
                  ))}
            ...
            {limitations && limitations.length > 0 && (
              <div style={{
                ...
                <div style={{ marginTop: 8, display: 'flex', flexDirection: 'column', gap: 3 }}>
                  {limitations.map((l, i) => (
                    <div key={i} style={{ fontSize: 11, color: '#3a3a3a', fontFamily: FONT, lineHeight: 1.5 }}>
                      · {l}
                    </div>
                  ))}
                </div>
